function f_date_eastern (p_year number )
return date
as
	li_p_year	integer;
	li_a		integer;
...
begin
		...
		ld_eastern := to_date(to_char(li_v_day, 'fm00')
				|| '-' || to_char(li_v_month, 'fm00')
				|| '-' || to_char(li_p_year, 'fm0000')
			,'dd-mm-yyyy');

		-- dbms_output.put_line ('In ' || li_p_year || ' Eastern on '
		-- 	|| to_char(li_v_day, 'fm00')
		-- 	|| '-' || to_char(li_v_month, 'fm00')
		-- 	|| '-' || to_char(li_p_year, 'fm0000')
		-- 	|| '; Ash Wednesday on ' || to_char(ld_eastern - 46, 'day dd-mm-yyyy') );
		-- Ash Wednesday - 46
		-- Ascension Day + 39
		-- Whitsun + 49
