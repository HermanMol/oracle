prompt *I: ScriptBOF tpb_string_stack_object.sql
...
/* [HM7A9_20240512_120400] */
	...
	is
		l_retval	varchar2(4000);	-- same as type t_string_varray50 -> varchar2(4000)
	begin
		l_retval := get(pi_idx);
		remove (pi_idx);
		return l_retval;
	...
	is
	begin
		return nvl(g_string_stack.first(),0);
	end first_idx;
	...
	is
	begin
		return nvl(g_string_stack.last(),0);
	end last_idx;
	...
	is
	begin
		g_string_stack := t_string_varray50();
		c_version := '1.00';
		return;
	end;
end;
/
prompt *I: ScriptEOF tpb_string_stack_object.sql
